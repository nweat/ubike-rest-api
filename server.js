...
var refWithinTaipei = function(lat, lon) {
  radius = 30000
  approx_center_lat_lng_taipei = {
    latitude: 25.056469,
    longitude: 121.513530
  }
  try {
    if (geolib.isPointInCircle({
        latitude: lat,
        longitude: lon
      }, approx_center_lat_lng_taipei, radius)) {
      return true
    }
  } catch (err) {
    return false
...
router.get('/ubike-station/taipei', function(req, res, next) {
  ...
  request({
    ...
    if (!error && response.statusCode === 200) {
      ...
      var input_coords = {
        'latitude': geolib.useDecimal(lat),
        'longitude': geolib.useDecimal(lng)
      }

      nearby_stations = req.data.map(function(el) {
        var you_bike_station_coords = {
          'latitude': geolib.useDecimal(el['lat']),
          'longitude': geolib.useDecimal(el['lng'])
        }
